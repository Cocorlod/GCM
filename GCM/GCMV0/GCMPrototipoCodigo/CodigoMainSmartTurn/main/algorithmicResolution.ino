...
static bool cellTimerRunning = false;

void beginCellTravel() {
    cellStartTime = millis();
    cellTimerRunning = true;
}

bool cellComplete() {
    if (!cellTimerRunning) return false;

    if (millis() - cellStartTime >= CELL_TRAVEL_TIME_MS) {
        cellTimerRunning = false;
        return true;
    }
