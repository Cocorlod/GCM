...
bool isGoalDetected() {
    return analogRead(IR_PIN) >= IR_THRESHOLD;
}
...
static DFSNode stackDFS[MAZE_MAX_CELLS];
static int sp = -1;

static void push(uint16_t cell, Heading h) {

    if(sp >= MAZE_MAX_CELLS - 1) return;
...
TurnDecision chooseDFS(Maze& maze, uint16_t current, Heading heading) {
    ...
    if(sp >= 0) {
        ...
        if(node.heading == rotateBack(heading)) return GO_FORWARD;
        if(node.heading == rotateLeft(heading)) return TURN_LEFT;
        if(node.heading == rotateRight(heading)) return TURN_RIGHT;
