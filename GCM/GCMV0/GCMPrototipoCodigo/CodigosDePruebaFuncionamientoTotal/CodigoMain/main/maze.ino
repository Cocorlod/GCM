...
static int16_t indexGrid[GRID_SIZE][GRID_SIZE];

static inline bool inGridBounds(int16_t gx, int16_t gy) {
...
void Maze::reset() {
    ...
    for(int16_t gx = 0; gx < GRID_SIZE; gx++) {
        for(int16_t gy = 0; gy < GRID_SIZE; gy++) {
            indexGrid[gx][gy] = -1;
        }
    }
}
...
int16_t Maze::cellAt(int16_t x, int16_t y) const {
    int16_t gx = x + GRID_OFFSET;
    int16_t gy = y + GRID_OFFSET;

    if(!inGridBounds(gx, gy)) return -1;

    return indexGrid[gx][gy];
}
